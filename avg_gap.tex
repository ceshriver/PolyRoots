...
\begin{proof}
	...
	\begin{enumerate}
		\item If an $(n-1)$-gap is at most $l+m+1$ wide, and all roots outside the gap are at least $L$ away, then the size of the gap increases; and
	...
	For all further iterations, we have that an $(n-1)$-gap either increases, or if it is larger than $l+m+1$, it may decrease. However, it may only decrease by at most $m$. Therefore the size of an $(n-1)$-gap will never drop below $l+\e$, contradicting that $l$ is the $\liminf$. It must then be that the $\liminf$ is infinite, so the size of the smallest $(n-1)$-gap approaches infinity.
\end{proof}
